...
def parse_arguments():
    ...
    parser.add_argument(
        "--config-folder",
        help=f"Path to configuration folder (default: '{def_config_folder}')",
    ...
    parser_full = subparsers.add_parser(
        "full-process", help="Download, transcribe, and process prompts"
    )
    parser_full.add_argument(
        "inputs",
        nargs="+",
        help="YouTube URLs, video IDs, or local file paths to process",
    )
    parser_full.add_argument(
        "--update-youtube",
        action="store_true",
        help="Update YouTube videos after processing (default: False)",
    )
    parser_full.add_argument(
        "--disable-improve-srt",
        action="store_true",
        help="Disable automatic improvement of transcribed SRT (default: False)",
        default=False,
    )
    ...
    parser_transcribe = subparsers.add_parser(
        "transcribe", help="Transcribe local audio files"
    )
    parser_transcribe.add_argument(
        "folders", nargs="+", help="Folders containing MP3 files to transcribe"
    )

    # Improve transcript
    parser_improve_transcript = subparsers.add_parser(
        "improve-srt", help="Improve automatically transcribed SRT"
    )
    parser_improve_transcript.add_argument(
        "folders", nargs="+", help="Folders containing transcribed files"
    )

    # Process prompts on transcriptions
    parser_prompts = subparsers.add_parser(
        "process-prompts", help="Process prompts on transcribed files"
    )
    ...
    parser_discussion = subparsers.add_parser(
        "generate-discussion-starters",
        help="Generate discussion starters for the next stream",
    )

    # Generate thumbnails
    parser_thumbnail = subparsers.add_parser(
        "generate-thumbnail", help="Generate thumbnail candidates for video"
    )
    parser_thumbnail.add_argument("folder", help="Folder containing video and metadata")

    # Update YouTube videos
    parser_update = subparsers.add_parser(
        "update-youtube", help="Update YouTube videos using folder details"
    )
    parser_update.add_argument(
        "folder", help="Folder containing file_details.txt and optional metadata files"
    )
...
def main():
    ...
    if (
        args.mode == "full-process" and args.update_youtube
    ) or args.mode == "update-youtube":
    ...
    if args.mode == "full-process":
        ...
        for input_item in args.inputs:
            if os.path.isfile(input_item):  # Check if the input is a local file
                ...
                # Convert to OGG
                ogg_file_path = downloader.convert_to_ogg(
                    input_item, video_folder, file_name
                )
            else:
                # Handle YouTube URLs or IDs
                audio_file, video_folder, title = downloader.download_youtube_video(
                    input_item, output_dir
                )
            ...
            is_download_video = config.get("download_video", "false").lower() == "true"
            ...
            if is_download_video:
                ...
                logger.info("Video download is disabled, skipping thumbnail generation")
        ...
        if thumbnail_paths:
            logger.info(
                f"Generated {len(thumbnail_paths)} thumbnail candidates in {args.folder}/thumbnails"
            )
